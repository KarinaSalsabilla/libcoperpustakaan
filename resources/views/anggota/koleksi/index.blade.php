...
<head>
  ...
  <style>
    ...
    *::after {
    ...
    .ph-spine:nth-child(1) {
      width: 14px;
      height: 72px;
      background: #60a5fa
    }

    .ph-spine:nth-child(2) {
      width: 11px;
      height: 88px;
      background: #a78bfa
    }

    .ph-spine:nth-child(3) {
      width: 17px;
      height: 64px;
      background: #fbbf24
    }

    .ph-spine:nth-child(4) {
      width: 10px;
      height: 78px;
      background: #34d399
    }

    .ph-spine:nth-child(5) {
      width: 14px;
      height: 58px;
      background: #fb923c
    }
    ...
    .sidebar-toggle i.chevron {
      transition: transform .25s
    }

    .sidebar-toggle.open i.chevron {
    ...
    [data-theme="dark"] .avail-opt:hover {
      background: rgba(59, 130, 246, .08)
    }

    .avail-opt input[type=radio] {
    ...
    .book-overlay {
      position: absolute;
      inset: 0;
      background: linear-gradient(to bottom, transparent 42%, rgba(0, 0, 0, .6))
    }
    ...
    [data-theme="dark"] .book-stok {
      color: #4ade80
    }

    [data-theme="dark"] .book-stok.habis {
      color: #f87171
    }
    ...
    [data-theme="dark"] .lc-stok.ok {
      color: #4ade80
    }

    [data-theme="dark"] .lc-stok.none {
      color: #f87171
    }
    ...
    .modal-cover {
      ...
      overflow: hidden;
      position: relative;
      box-shadow: -5px 8px 26px rgba(37, 99, 235, .2);
    }
    ...
    [data-theme="dark"] .modal-avail.ok {
      color: #4ade80
    }

    [data-theme="dark"] .modal-avail.none {
      color: #f87171
    }
    ...
    <div class="page-header">
      ...
      <div class="ph-left">
        <div class="ph-eyebrow">
          <div class="ph-eyebrow-dot"></div>Perpustakaan Digital
        </div>
        <h1 class="ph-title">Koleksi <em>Buku</em></h1>
        <p class="ph-sub">Temukan dan pinjam buku favoritmu dari <strong>{{ $ebooks->total() }} judul</strong> yang
          tersedia dalam koleksi kami.</p>
      </div>
      <div class="ph-stats">
        <div class="ph-stat">
          <div class="ph-stat-num">{{ $ebooks->total() }}</div>
          <div class="ph-stat-label">Total Buku</div>
        </div>
        <div class="ph-stat">
          <div class="ph-stat-num">{{ $kategoris->count() }}</div>
          <div class="ph-stat-label">Kategori</div>
        </div>
        <div class="ph-stat">
          <div class="ph-stat-num">{{ $ebooks->where('jumlah_ebook', '>', 0)->count() }}</div>
          <div class="ph-stat-label">Tersedia</div>
        </div>
      </div>
    </div>
    ...
    <div class="layout">
      ...
      <aside>
        <button class="sidebar-toggle" id="sidebarToggle">
          <span style="display:flex;align-items:center;gap:8px;"><i class="fas fa-sliders-h"
              style="color:var(--blue-500);"></i><span>Filter &amp; Cari</span></span>
          ...
          <div class="sidebar-collapsible" id="sidebarCollapsible">
            ...
            <div class="fs-body">
              ...
              <div class="fs-section">
                ...
                <div class="cat-list">
                  <button class="cat-item active" data-cat="semua">Semua<span
                      class="cat-count">{{ $ebooks->total() }}</span></button>
                  @foreach($kategoris as $kat)
                    <button class="cat-item" data-cat="{{ $kat->id_kategori }}">{{ $kat->nama_kategori }}<span
                        class="cat-count">{{ $kat->ebooks_count ?? 0 }}</span></button>
                  @endforeach
                </div>
              </div>
      ...
      <div class="main-content">
        <div class="toolbar">
          <span class="result-info" id="resultInfo">Menampilkan <strong>{{ $ebooks->count() }}</strong> dari
            {{ $ebooks->total() }} buku</span>
        ...
        <div class="book-grid" id="bookGrid">
          @forelse($ebooks as $book)
            ...
            {{-- GRID CARD --}}
            <a href="#" class="book-card grid-item" data-cat="{{ $book->id_kategori }}" data-stok="{{ $stok }}"
              ...
              data-cover="{{ $hasImg ? Storage::url($book->cover) : '' }}" data-color="{{ $color }}"
              data-tahun="{{ $book->tahun_terbit ?? '-' }}" data-isbn="{{ $book->isbn ?? '-' }}"
              data-detail-url="{{ route('anggota.buku.show', $book->id_buku) }}" onclick="bukaModal(event,this)">
              <div class="book-cover-wrap">
                @if($hasImg)
                  <img src="{{ Storage::url($book->cover) }}" class="book-cover-img" alt="{{ $book->judul_buku }}">
                @else
                  <div class="book-cover-ph"
                    style="background:{{ $color }};width:100%;height:100%;position:absolute;inset:0;display:flex;align-items:flex-end;padding:12px 10px;">
                    <div class="book-overlay"></div>
                    <div class="book-cover-title" style="position:relative;z-index:1;">
                      {{ Str::limit($book->judul_buku, 30) }}</div>
                  </div>
                @endif
                <div class="book-spine"></div>
                @if($hasImg)
                <div class="book-overlay"></div>@endif
                <div class="avail-dot {{ $stok > 0 ? 'ok' : 'none' }}"></div>
                @if($stok > 0)
                  <a href="{{ route('anggota.buku.show', $book->id_buku) }}" class="hover-pinjam"
                    onclick="event.stopPropagation()"><i class="fas fa-book-open"></i> Pinjam</a>
                @else
              ...
              <div class="book-footer">
                ...
                <span
                  class="book-stok {{ $stok > 0 ? '' : 'habis' }}">{{ $stok > 0 ? $stok . ' tersedia' : 'Habis' }}</span>
              </div>
            </a>

            {{-- LIST CARD --}}
            <a href="#" class="list-card list-item" style="display:none;" data-cat="{{ $book->id_kategori }}"
              data-stok="{{ $stok }}" data-judul="{{ strtolower($book->judul_buku) }}"
              data-penulis="{{ strtolower($book->pengarang) }}" data-full-judul="{{ $book->judul_buku }}"
              data-full-penulis="{{ $book->pengarang }}" data-kategori="{{ $book->kategori->nama_kategori ?? '-' }}"
              data-sinopsis="{{ Str::limit($book->sinopsis ?? 'Deskripsi belum tersedia.', 300) }}"
              data-cover="{{ $hasImg ? Storage::url($book->cover) : '' }}" data-color="{{ $color }}"
              data-tahun="{{ $book->tahun_terbit ?? '-' }}" data-isbn="{{ $book->isbn ?? '-' }}"
              data-detail-url="{{ route('anggota.buku.show', $book->id_buku) }}" onclick="bukaModal(event,this)">
              <div class="lc-cover" style="background:{{ $color }}">
                @if($hasImg)<img src="{{ Storage::url($book->cover) }}" alt="">@else<div class="lc-cover-ph">📚</div>
                @endif
              ...
              <div class="lc-info">
                ...
                @if($book->sinopsis)
                <div class="lc-desc">{{ Str::limit($book->sinopsis, 130) }}</div>@endif
              </div>
              <div class="lc-actions">
                <span class="lc-stok {{ $stok > 0 ? 'ok' : 'none' }}">{{ $stok > 0 ? $stok . ' eks.' : 'Habis' }}</span>
                <a href="{{ route('anggota.buku.show', $book->id_buku) }}" class="lc-btn lc-btn-ghost"
                  onclick="event.stopPropagation()"><i class="fas fa-eye"></i></a>
                @if($stok > 0)
                  <a href="{{ route('anggota.buku.show', $book->id_buku) }}" class="lc-btn lc-btn-pinjam"
                    onclick="event.stopPropagation()"><i class="fas fa-book-open"></i> Pinjam</a>
                @else
      ...
      <div class="modal-inner">
        ...
        <div class="modal-info">
          ...
          <div class="modal-metas">
            <div>
              <div class="mm-label">Tahun Terbit</div>
              <div class="mm-val" id="mTahun">—</div>
            </div>
            <div>
              <div class="mm-label">ISBN</div>
              <div class="mm-val" id="mIsbn">—</div>
            </div>
          </div>
